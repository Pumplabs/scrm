...
import {
  AddGroupChatTagGroup,
  GetGroupChatTagGroupList,
  EditGroupChatTagGroup,
  AddGroupChatTag,
  DelGroupChatTagGroup,
} from 'services/modules/groupChatTag'
import AddTagModal from './components/AddTagModal'
import { useModalHook, useInfiniteHook, useGetWindowInfo } from 'src/hooks'
...
export default () => {
  ...
  const {
    ...
    pageSize: 10,
    request: GetGroupChatTagGroupList,
    form: searchForm,
  })

  const { run: runAddGroupChatTagGroup } = useRequest(AddGroupChatTagGroup, {
    manual: true,
  ...
  // 新增群聊标签
  const { run: runAddGroupChatTag } = useRequest(AddGroupChatTag, {
    manual: true,
    onBefore: () => {
      setConfirm()
    },
  ...
  const { run: runEditGroupChatTagGroup } = useRequest(EditGroupChatTagGroup, {
    manual: true,
    onBefore: () => {
      setConfirm()
    },
  ...
  const { run: runDelGroupChatTagGroup } = useRequest(DelGroupChatTagGroup, {
    manual: true,
    onBefore: () => {
      setConfirm()
    },
  ...
  const onAddModalOk = (values) => {
    ...
    if (visibleMap.addVisible) {
      ...
      runAddGroupChatTagGroup(params)
    }
    if (visibleMap.editVisible) {
      ...
      runEditGroupChatTagGroup(params)
    }
  }
  ...
  const onSaveTagItem = (text) => {
    ...
    if (modalInfo.type === 'addTag') {
      runAddGroupChatTag({
        name: text,
        groupChatTagGroupId: modalData.groupInfo.id,
      }, { groupIdx: modalData.groupIdx})
    } else if (modalInfo.type === 'editTag') {
      const params = {
        extCorpId: modalData.extCorpId,
        id: modalData.groupInfo.id,
        name: modalData.groupInfo.name,
        updateTags: [
          {
            id: modalData.id,
            name: text,
  ...
  const onEditTagItem = (groupItem, data, groupIdx) => {
    openModal('editTag', {
      ...data,
      groupIdx,
  ...
  const onRemoveTagItem = (data, groupIdx) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        runEditGroupChatTagGroup(params, { actionName: '删除', groupIdx })
      },
    })
  }
        ...
        <InfiniteList
          ...
          renderItem={(ele, idx) => (
            <TagGroupCard
              ...
              onEditTagItem={(...args) => onEditTagItem(ele, ...args)}

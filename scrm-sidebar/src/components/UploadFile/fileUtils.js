...
export const validateFileNameLenIsOver = ({ file }, { maxNameLen }) => {
  const { name } = file;
  const nameLenIsNotOver = validateFileNameSize(name, maxNameLen);
  return nameLenIsNotOver;
}
 ...
 */
export const validateNameIsExist = ({ file, fileList, uploadedFiles = [] }) => {
  const { name } = file;
  // 判断文件是否存在
...
export const getFileParams = (fullName = '') => {
  const idx = fullName.lastIndexOf('.')
  if (idx > -1) {
    const fileType = fullName.slice(idx)
    const fileName = fullName.slice(0, idx)
